...
class Beam(object):
    def __init__(self, size, n_best=1, cuda=False, vocab=None,
        ...
        self.scores = Variable(self.tt.FloatTensor(size).zero_())
        self.allScores = []
        ...
        self.nextYs = [Variable(self.tt.LongTensor(size)
        ...
        * `attnOut`- attention at the last step
        ...
        if len(self.prevKs) > 0:
            ...
            for i in range(self.nextYs[-1].size(0)):
                if (self.nextYs[-1][i] == self._eos).all():
                    beamLk[i] = -1e20
        else:
            beamLk = wordLk[0]
        ...
        for i in range(self.nextYs[-1].size(0)):
            if (self.nextYs[-1][i].data == self._eos).all():
                s = self.scores[i]
                if self.globalScorer is not None:
        ...
        if (self.nextYs[-1][0].data == self._eos).all():
            # self.allScores.append(self.scores)
            self.eosTop = True
